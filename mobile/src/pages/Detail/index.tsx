...
const Detail = () => {
  ...
  useEffect(() => {
    api.get(`/collect_points/${routeParams.point_id}`).then(response => {
  ...
  function handleComposeMail() {
    MailComposer.composeAsync({
      subject: 'Tenho interesse na coleta de resíduos',
  ...
  if (!data.point) {
    return null; //fazendo um loading
  }
    ...
    <SafeAreaView style={{ flex: 1 }}>
      <View style={styles.container}>
        <TouchableOpacity onPress={handleNavigateBack}>
          <Icon name='arrow-left-circle' color='#34CB79' size={24} />
        </TouchableOpacity>
